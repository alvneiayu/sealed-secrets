...
Our images are being signed using cosign. The signatures have been saved in our [GitHub Container Registry](https://github.com/bitnami-labs/sealed-secrets/pkgs/container/sealed-secrets/signs).

It is pretty simple to verify the images, here you have an example:

```bash
# export the COSIGN_VARIABLE setting up the GitHub container registry signs path
export COSIGN_REPOSITORY=ghcr.io/alvneiayu/sealed-secrets/signs

cosign verify --key .github/workflows/cosign.pub ghcr.io/bitnami-labs/sealed-secrets:latest
```
