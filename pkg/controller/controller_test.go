...
// TestAttemptUnsealIgnoresTemplate is a regression test for a decryption
// oracle: /v1/verify (backed by AttemptUnseal) must decide validity purely
// from whether encryptedData decrypts, never from spec.template.data
// template execution, since template.data is not covered by the AEAD label
// and an attacker can pair it with a victim's genuine metadata/encryptedData.
func TestAttemptUnsealIgnoresTemplate(t *testing.T) {
	...
	// Simulate an attacker pairing the victim's genuine metadata and
	// encryptedData with an attacker-controlled, always-failing template.
